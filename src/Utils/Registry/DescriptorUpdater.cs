...
public class DescriptorUpdater
{
    ...
    public static void UpdateAasDescriptor(AssetAdministrationShellDescriptor aasDescriptor, EditorDescriptor editorDescriptor, AssetAdministrationShell aas, List<Submodel> submodels)
    {
        ...
        aasDescriptor.SubmodelDescriptors?.ForEach((smDescriptor) =>
        {
            var editorDescriptorEntry = editorDescriptor.SubmodelDescriptorEntries.Find((entry) => entry.OldId == smDescriptor.Id);
        ...
        {
            if (aasDescriptor.SubmodelDescriptors?.Find((smDescriptor) => smDescriptor.Id == submodel.Id) == null)
            {
                var editorDescriptorEntry = editorDescriptor.SubmodelDescriptorEntries.Find((entry) => entry.OldId == submodel.Id);
                if (editorDescriptorEntry != null)
                {
                    aasDescriptor.SubmodelDescriptors?.Add(DescriptorCreator.CreateSubmodelDescriptorWithFullUrl(submodel, editorDescriptorEntry.Endpoint));
                }
            }
        });

        // Endpunkt anpassen
        var oldIdBase64 = editorDescriptor.AasDescriptorEntry.OldId.ToBase64UrlEncoded(Encoding.UTF8);
        var newIdBase64 = editorDescriptor.AasDescriptorEntry.NewId.ToBase64UrlEncoded(Encoding.UTF8);
        aasDescriptor.Endpoints.ForEach((endpoint) =>
        {
            if (endpoint.ProtocolInformation?.Href.Contains(oldIdBase64) == true)
            {
                endpoint.ProtocolInformation.EndpointProtocol = endpoint.ProtocolInformation.EndpointProtocol?.Split("://")[0] ?? "https";
                endpoint.Interface = "AAS-3.0";
                endpoint.ProtocolInformation.Href = endpoint.ProtocolInformation.Href.Replace(oldIdBase64, newIdBase64);
            }
        });
    }

    public static void UpdateSubmodelDescriptor(SubmodelDescriptor smDescriptor, Submodel submodel, string oldId)
    {
        ...
        smDescriptor.Endpoints.ForEach((endpoint) =>
        {
            if (endpoint.ProtocolInformation?.Href.Contains(oldIdBase64) == true)
            {
                endpoint.ProtocolInformation.EndpointProtocol = endpoint.ProtocolInformation.EndpointProtocol?.Split("://")[0] ?? "https";
