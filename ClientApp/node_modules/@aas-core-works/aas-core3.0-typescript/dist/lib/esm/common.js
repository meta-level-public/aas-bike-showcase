 ...
 */
export function every(iterable) {
 ...
 */
export function* map(iterable, mappingFunction) {
  // NOTE (mristin, 2022-11-24):
  // We introduce this function so that we can keep the constraint verification
  // purely functional.
...
export function base64Decode(text) {
  ...
  for (let i = 0; i < len; i += 4) {
    ...
    if (charCode0 >= base64LookupLen) {
      return new Either(null, "Expected a valid character from base64-encoded string, " + `but got at index ${i}: ${text[i]} (code: ${charCode0})`);
    }
    ...
    if (charCode1 >= base64LookupLen) {
      return new Either(null, "Expected a valid character from base64-encoded string, " + `but got at index ${i + 1}: ${text[i + 1]} (code: ${charCode1})`);
    }
    ...
    if (charCode2 >= base64LookupLen) {
      return new Either(null, "Expected a valid character from base64-encoded string, " + `but got at index ${i + 2}: ${text[i + 2]} (code: ${charCode2})`);
    }
    ...
    if (charCode3 >= base64LookupLen) {
      return new Either(null, "Expected a valid character from base64-encoded string, " + `but got at index ${i + 3}: ${text[i + 3]} (code: ${charCode3})`);
    }
